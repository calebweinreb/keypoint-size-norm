...
Before deriving EM steps for our full desired context, we begin with a paired-down model for only one animal, where we take $F = \mathrm{Id}$ and $R = \eps I$
\begin{align}
    z_t & \sim \text{Cat}(\pi) &
...
There are a couple of considerations also for how we will treat $\pi$, due to the constraint $\sum \pi = 1$:
\begin{itemize}
    \item We can work where possible to arrive at analytical optima for $\pi$. The results should be similar across models, always corresponding to the vanilla GMM result ${\pi_k}_{\text{new}} = \frac{1}{N}\sum_t q_t(k)$. However it is not clear what will happen when priors are introduced. \textbf{We will begin with this approach.}
    \item The model could be reformulated with $\pi = \bar{\pi} / \sum\bar{\pi}$, though standard formulas for working with priors may need to be updated accordingly.
    \item Once a Dirichlet prior is introduced, $\pi$ could be treated as a latent, with $\alpha$ the replacement parameter, since it is unconstrained. We may then however have to impose a prior on $\alpha$.
\end{itemize}
...
In this section we present the rersults necessary to implement EM for the paired down Gaussian mixture model of Section \ref{sec:paired-down-gmm}. For details, see Section \ref{sec:em-paired-down-derive}. The results of the E-step are implicit in the formulation of the objective function, but we will use the following constants that arise during the E-step:
\begin{align}
...
\end{align}
which arise as Equations \ref{eq:sigma-star}, \ref{eq:mu-star}, and \ref{eq:y-marg-post} respectively in the derivation. We optimize $A$ with $\pi$ fixed to its analytical optimum (Equation \ref{eq:pi-anlt-opt})
\begin{align}
    {\pi^*_k}_{\text{new}} = \frac{\pi_{k}^* \sum_t P(y_t | k, \theta^*)}{\sum_{z} \pi_{z}^* \sum_t P(y_t | z, \theta^*)}.
\end{align}
Subject to this constraint, proceed by gradient ascent on the function
\begin{align}
    J(\theta) = \sum_t\sum_z -\frac{\pi_z^*\,P(y_t\mid z)}{2} \bigg( 
      & \log\abs{R} + \NN(\mu_{z,t}^* \mid y_t, R) + \Tr\bc{\Sigma_{z, t}^* R\inv} \\
    + & \log\abs{Q_z} + \NN(\mu_{z,t}^* \mid m_z, Q_z) + \Tr\bc{\Sigma_{z, t}^* Q_z\inv} \bigg)
\end{align}
...
\label{sec:em-paired-down-derive}
\section{Derivaiton of EM for the paired-down mixture model}

In this section we expand upon parameter inference procedure outlined in Section \ref{sec:em-general} for the paired down Gaussian mixture model of \ref{sec:em-results-paired-down}.

\textbf{E step:}\ \ In the ``expectation'' step, we begin from Equation \ref{eq:general-qt} to derive the auxilliary distribution $q_t(x, z)$. The first action will be to remove unneccesary conditional terms and apply the assumed distributions of the model:
\begin{align}
...
\textit{Optimum in $\pi$.}\ \ The mixture weights parameter is subject to the constraint $\sum_z \pi_z = 1$, and the form of its optimum will be similar across models, so we pursue analytical optimization for this parameter. To do so, we introduce a lagrangian $L = A + \lambda \pn{1 - \sum\pi}$. For each weight $\pi_k$, terms $z\neq k$ in the sum over $z$ of Equation \ref{eq:em-int-argmax} are constant
\begin{align}
    \frac{\pd L}{\pd \pi_k}
    &= -\lambda + \frac{\pd }{\pd \pi_k} \sum_{t}\sum_z\int_{x}  q_t(z, k) \log P(z \mid \pi)\\
    &= -\lambda + \sum_{t}\int_{x}  q_t(x, k) \frac{\pd }{\pd \pi_k} \log \pi_{k} \\
    &= -\lambda + \pi_{k}^* \sum_t P(y_t | k, \theta^*) \int_x \NN(x \mid \mu^*_{k, t}, \Sigma^*_{k, t})\frac{1}{\pi_k} \\
    &= -\lambda + \pi_{k}^* \sum_t P(y_t | k, \theta^*) \bigg/ \pi_k. \\
\end{align}
Solving for zeros of this partial derivative yields $\pi_k = \pi_{k}^* \sum_t P(y_t | k, \theta^*) / \lambda$, and using the constraint $\sum\pi = 1$ it is clear that $\lambda$ must be a normalization constant. Therefore we arrive at the optima for $\pi_k$:
\begin{align}
    \label{eq:pi-anlt-opt}
    {\pi^*_k}_{\text{new}} = \frac{\pi_{k}^* \sum_t P(y_t | k, \theta^*)}{\sum_{z} \pi_{z}^* \sum_t P(y_t | z, \theta^*)}.
\end{align}
