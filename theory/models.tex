...
In the Gaussian mixture model, our discrete pose state is categorical with weights subject to a heirarchical Dirichlet prior with hyperparameters $\hat\gamma, \hat\beta, \beta_{\text{max}}$, and our continuous pose state is normal, with mean and covariance conditional on the discrete state.
\begin{align*}
    ...
    &&& \beta_n = \text{softmax}( \beta_{\text{max}}\text{tanh}( \bar\beta_n / \beta_{\text{max}})),\ \beta_n \sim \text{Dir}(\hat\beta\;\gamma) \\
