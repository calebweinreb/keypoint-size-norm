...
\begin{align}
    \log \NN(y\mid C_n(x - d_n), R) &= \log\pn{\frac{Z_{R}}{Z_{C_n\inv R {C_n^T}\inv}} \NN\pn{x - d_n\mid C_n\inv y,\; C_n\inv R\, {C_n^T}\inv}} \\ &= \log Z_{R} - \frac{1}{2}\norm{(x - d_n) - C_n\inv y}^2_{C_n\inv R\, {C_n^T}\inv}
    \label{eq:cn-swap}
\end{align}
...
Let $L$ be an integer d.o.f. hyperparameter giving the number of axes to be rotated, and define the subject-wise parameter tuple $\phi_n = (\alpha_n, s_n, U, \hat{U}_n)$
\begin{itemize}
