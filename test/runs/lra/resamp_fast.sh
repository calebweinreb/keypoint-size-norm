...
#
# Base: resamp_long
# Modified: Can go faster without instability by not reinitializing the
# optimizer parameters each mstep.
...
base_fit="$base_fit:n_steps=100:learning_rate=30:return_param_hist=mstep:mstep_reinit_opt=False"
run_name=resamp_fast
...
python3 $SCRIPT/test_arch.py \
    ...
    --fit_cfg "$base_fit:update_blacklist=('posespace/means','posespace/cholesky')"
