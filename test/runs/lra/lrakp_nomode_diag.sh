...
#
# Base: lrakp_diag
# Altered to have a larger subjectwise mean update and *no* subjectwise mode
# update in the data generation.

# with $HOME/projects/mph/generative/test/runs/env.sh

base_data="path=$HOME/projects/mph/data_explore/data"
base_data="$base_data:age_blacklist=[9,12]:subsample=15"
src_sess="3wk_m0"
base_fit="log_every=1:batch_size=None:scale_lr=True:mstep_n_steps=400"
base_fit="$base_fit:n_steps=3:learning_rate=1:return_param_hist=mstep"
run_name=lrakp_nomode_diag
...
python3 $SCRIPT/test_arch.py \
    ...
    --data_cfg "$base_data:src_sess=$src_sess:n_subj=2:param_sample.update_std=0:param_sample.offset_std=3" \
