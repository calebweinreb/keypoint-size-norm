...
#
# Base: resamp_base.sh
# Modified: to test non-normality as an issue causing nonzero morph, adding more
# GMM components
...
base_fit="$base_fit:n_steps=60:learning_rate=8:return_param_hist=mstep"
run_name=resamp_long-m
...
python3 $SCRIPT/test_arch.py \
    ...
    --fit_cfg "$base_fit:mstep_reinit_opt=False::mstep_stop_window=100"
...
python3 $SCRIPT/plot_battery.py \
    ...
    reconst-err reconst_err "colorby=bhv:ref_sess=$id_sess" \
    --do ${2:--}
