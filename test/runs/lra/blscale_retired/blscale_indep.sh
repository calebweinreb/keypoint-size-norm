...
run_name=blscale_indep-fast
# normal mode: very slow
elif [ 'hyper' = "$2" ]; then
specific_fit="n_steps=4:learning_rate=1e1"
run_name=blscale_indep-hyper
else
specific_fit="n_steps=20:learning_rate=1e-1"
run_name=blscale_indep-slow
fi
...
python3 $SCRIPT/test_arch.py \
    ...
    --data_cfg "$keypt_dir:$downsamp:$age_blk:src_sess=$src_sess:output_indep" \
...
python3 $SCRIPT/plot_battery.py \
    ...
    lra lra_params "colorby=tgt_age" \
    gmm gmm_params "colorby=tgt_age" \
