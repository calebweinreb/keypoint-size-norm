...
base_fit="$base_fit:n_steps=10:learning_rate=1:return_param_hist=mstep"
run_name=lrakp_jtr_nomode
nomode="param_sample.update_std=0:param_sample.offset_std=3"
...
python3 $SCRIPT/test_arch.py \
    ...
    --data_cfg "$base_data:src_sess=$src_sess:n_subj=2:jitter=1e-2:$nomode" \
...
python3 $SCRIPT/plot_battery.py \
    ...
    gmm gmm_params "colorby=body:eigs" \
