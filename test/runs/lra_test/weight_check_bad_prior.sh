...
base_fit="$base_fit:n_steps=30:scale_lr=True:learning_rate=8:mstep_reinit_opt=False:return_param_hist=mstep"
ref_sess=0
run_name=weight_check_bad_prior
dset="pose.hyperparam.L=2:n_frames=1000:n_subj=1"
...
dset="$dset:pose.param.subj_log=[[4,1]]:pose.param.pop_log=[1,1]:pose.param.means=[[-1,0],[1,0]]:pose.param.var=[0.3,0.3]"
dset="$dset:morph.param.dmode=[[[0],[0]]]:morph.param.dofs=[[0,0]]"
...
python3 $SCRIPT/test_arch.py \
    ...
    --fit_cfg "$base_fit:update_blacklist=('morph/*',)"
